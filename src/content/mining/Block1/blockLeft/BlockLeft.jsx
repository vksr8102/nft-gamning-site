...
function BlockLeft({activeResult,result,avalible,active,setActive,setAvalible,handleCheckboxChange,handleActive,selectedIds,fetchMoremining,loading,fetchMoreActivemining,handleAvalible,handleSelectAll,selectAll,handleCancleAll}) {
...
//    console.log(activeResult,active);
  return (
    <div className='sm:h-[550px] md:[650px] lg:h-[800px] h-[450px] '>
        ...
        <div className={`flex items-center  justify-between sm:gap-[20%] gap-[1%] xl:gap-[4%] xl:mt-[0%] md:gap-[12%] mb-1 2xl:gap-[10%] w-3/5 pr-2 sm:pr-[100px]  lg:gap-[1%] lg:mt-[3px] md:mt-[.5px] sm:mt-1  ml-[20px] mt-[-1%] sm:ml-[60px] lg:ml-[50px] lg:pr-[60px]  managegap margin `}>
      ...
      {active? <img src={'/ACTIVE_selected.png'} className={` h-[30px] w-[100px] md:h-[40px] md:w-[180px] xl:h-[40px] xl:w-[180px] lg:w-[100px] lg:h-[30px]  cursor-pointer  manageimg active`}  />:
        ...
        <div id='infinitescrool' className=' relative  scroll-hide overflow-auto md:h-[82%] lg:h-[83%]  sm:h-[83.5%] h-[83.5%] sm:mt-[15px]  sm:mr-[80px] mt-[8px]  ml-[45px] mr-[35px] sm:ml-[60px] md:mr-20 md:ml-16  lg:mx-20 lg:mt-6 lg:pt-0  pt-2 pb-10 screencard'>
...
<Spiner/>
        ...
        {result&&result?.data&&result?.data?.rows&&result?.data?.rows?.length===0 ?  
         ...
         loader={loading}
         scrollableTarget="infinitescrool"
       >
        {result&&result.data&&result.data?.message&&result?.data?.message !==null &&
                <div className='md:h-[400px] w-full flex justify-center items-center z-50  '>
                    <div className='md:h-[300px] md:w-[400px] border-2 p-4 mt-20 border-[#FF7200] rounded-xl box-shodow flex justify-center items-center'>
        ...
        <div className='flex justify-end pr-4 pb-5 py-1 md:pr-4 sm:pr-10'>
...
{ result&&result.data&&result.data.rows&&result?.data?.rows?.length>0 && result.data.rows.map((item)=>(
    ...
    <div  className='w-full lg:w-full md:w-full sm:w-full xl:w-full border-[3px] border-[#FF7200] p-[2px] cursor-pointer' onClick={(e)=>handleCheckboxChange(item.id,item?.free_action)}>
<Box sx={{border:"1px solid #fff",position:"relative"}}>

    <input type="checkbox" checked={selectedIds.includes(String(item.id))} value={selectedIds.includes(String(item.id))} name="" id='' className='absolute top-1 left-1 cursor-pointer z-50 ' onChange={() => handleCheckboxChange(item.id,item?.free_action)} />
    <img src={item&&item.nft_web_image} className=' xl:h-[70%] lg:h-[70%] md:h-[70%] h-[70%] sm:h-[70%] h-[138px] w-[100%]  object-cover'  />
</Box>
<div className=' '>
<div>
    <p className='text-center text-white  font-semibold  text-[10px] 2xl:text-[16px] uppercase  '>Name: <span className='text-[#44B6A1]'>{item&&item.nft_name}</span> </p>
</div>
<div className='p-[3px]'>
<p className=' text-white text-[8px] 2xl:text-[14px]  font-semibold uppercase'>POSITION :  <span className='text-[#44B6A1]'> {item&&item?.nft_subclass}</span> </p>
<p className=' text-white text-[8px] 2xl:text-[14px]   font-semibold uppercase'>Daily Cost :  <span className='text-[#44B6A1]'> {item&&item?.base_cost}</span> </p>
<p className=' text-white text-[8px] 2xl:text-[14px]   font-semibold uppercase'>Daily Production :   <span className='text-[#44B6A1]'> {item?.base_production} </span> </p>
<p className=' text-white text-[8px] 2xl:text-[14px]   font-semibold uppercase'>Tax % :   <span className='text-[#44B6A1]'> {item?.base_tax} </span> </p>
<p className=' text-white text-[8px] 2xl:text-[14px]   font-semibold uppercase'>Theft % :   <span className='text-[#44B6A1]'> {item?.base_theft} </span> </p>
